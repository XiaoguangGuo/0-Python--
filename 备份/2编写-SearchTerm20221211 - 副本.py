...
for countryname in SearchTermAll_Good_Country_list:   #遍历searchTemgood里的国家
    ...
    if n==1:
        ...
        Bulkfile_SearchTerm_Add_draft99=pd.DataFrame(columns=["Record ID","Record Type","Campaign ID","Campaign","Campaign Daily Budget","Portfolio ID","Campaign Start Date","Campaign End Date","Campaign Targeting Type","Ad Group","Max Bid","Keyword or Product Targeting","Product Targeting ID","Match Type","SKU","Campaign Status","Ad Group Status","Status","Impressions","Clicks","Spend",	"Orders","Total units",	"Sales	ACoS","Bidding strategy","Placement Type","Increase bids by placement"])
        ...
        for Stoi in range(0,len(SearchTermAll_Good_Country)):#遍历词#for A0
            ...
            if Stoi_keyword.startswith('b0'):
                ...
                continue
            if length_type==0:
               print("这个好词对应的Camaign没有记录，生成新的Campaign")
               #生成一个新的campaign
               #?生成一个campaign
               #？生成一个Ad g
               ...
               Bulkfile_SearchTerm_Add_draft99=Bulkfile_SearchTerm_Add_draft99.append({"Record Type":"Ad ","Campaign":pr66_sku,"Ad Group":pr66_sku,"Max Bid":Maxbid_Adgroup,"SKU":bulkfile_Campaign_SKUMax66,"Campaign Status":"enabled","Ad Group Status":"enabled"},ignore_index = True)
...
#####################################如果好词从Auto广告产生########################Auto#################################################################################################                
                ...
                #找到历史花费最多的SKU
                    ...
                    bulkfile_Campaign_SKU_list=bulkfile_Campaign_SKUMax1.loc[(bulkfile_Campaign_SKUMax1["Country"]==countryname)&(bulkfile_Campaign_SKUMax1["Campaign"]==Stoi_Campaign)&(bulkfile_Campaign_SKUMax1['Ad Group']==Stoi_AdGroup),"SKU"].to_list()
                    print("SKUlist",bulkfile_Campaign_SKU_list)
                    bulkfile_Campaign_SKUMax2=bulkfile_Campaign_SKU_list[0]
                ...
                ##上面为找出这个自动广告中花费最多的SKU，后面新开的广告就用到这个SKU：bulkfile_Campaign_SKUMax2
                    ...
                    bulkfileSKU_Manual_Campaigndf=bulkfile.loc[(bulkfile["Campaign"].isin(bulkfileSKU_Campaign_List)==True)&(bulkfile['Record Type']=="Campaign")&(bulkfile["Campaign Targeting Type"]=="Manual"),"Campaign"]
                #找到所有手动的 
                    ...
                    if bulkfileSKU_Manual_Campaign_Length==0: #if-A1
                        ...
                        pr4number=0
                        for bulkfileSKU_Manual_Campaign in bulkfileSKU_Manual_Campaign_List:#遍历为这个花费最多的SKU对应的Bulkfile中的所有手动Campaign的list# for A1
                            print("遍历手动Campaign")
