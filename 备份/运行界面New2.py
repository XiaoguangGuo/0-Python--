...
# 创建库存管理菜单
menu_inventory = tk.Menu(menubar, tearoff=0)
for i in range(3):
    file_path = f'D:/0-Python程序/inventory_{i}.py'
    if os.path.isfile(file_path):
        menu_inventory.add_command(label=f'库存管理{i}', command=lambda file_path=file_path: subprocess.Popen(f'
