...
#创建汇总表
pinpaibulkhzWorkbook=Workbook()
sheet=pinpaibulkhzWorkbook.active
Listcol=['Record ID','Record Type','Campaign ID','Campaign','Campaign Type','Ad Format','Budget','Portfolio ID','Campaign Start Date','Campaign End Date','Budget Type',	'Landing Page Url', 'Landing Page ASINs','Brand Name','Brand Entity ID','Brand Logo Asset ID','Headline', 'Creative ASINs', 'Media ID',	'Automated Bidding','Bid Multiplier','Ad Group','Max Bid',  'Keyword',	'Match Type','Campaign Status','Serving Status','Ad Group Status','Status', 'Impressions','Clicks','Spend', 'Orders','Total Units','Sales','ACoS','Placement Type','Country','日期','周数']
sheet.append(Listcol)
pinpaibulkhzWorkbook.save(r'D:\运营\周bulk广告数据汇总表NEWBrand.xlsx')




sheetnames=pinpaibulkhzWorkbook.sheetnames
print("表名",sheetnames)
print(pinpaibulkhzWorkbook.sheetnames)
sheet=pinpaibulkhzWorkbook[sheetnames[0]]
print(sheet.title)
...
#判断是否是10周内的数据，如果是就读数据，如果不是，则显示此文件不读。
    if delta<11:
            sourcedata=pd.read_excel(bulkdatafilepath+str(bulkdatafile),engine="openpyxl",sheet_name=2).assign(Country=os.path.basename(bulkdatafile).split('_')[0], 日期=os.path.basename(bulkdatafile).split('-')[4])
...
for i in range(2,sheet.max_row+1):
    ...
    sheet.cell(row=i,column=39).value=datetime.datetime.strptime(b,'%Y%m%d')
    sheet.cell(row=i,column=39).number_format = 'yyyy-mm-dd hh:mm:ss'
    ...
    print(maxtime,b,c)     
        #caozuoriqi= datetime.datetime.strptime(sheet.cell(row=i, column=39).value, "%Y%m/%d/")
        #print(caozuoriqi)
    #zhoushu=(('2021-10-30').dt.days-(cell(row=i,column=39).value).dt.days)//7+1
