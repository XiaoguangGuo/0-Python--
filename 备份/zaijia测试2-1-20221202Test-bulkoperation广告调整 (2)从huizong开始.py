...
for bulkoperationfile in os.listdir(bulkoperationfilespath):
    ...
    dfbulkfile=pd.read_excel(bulkoperationfilespath+str(bulkoperationfile),engine="openpyxl",sheet_name=1)#用PD读取待操作的某国bulk文件。!!!!!!!!!!!sheetname要改成1
...
#对于campaign和adgroup有一个是pause的，原来其他的词status为Pause
        ...
        dfbulkfile.loc[(dfbulkfile["Campaign"]==boi_campaign) & (dfbulkfile["Ad Group"]==boi_ADgroup)& (~(dfbulkfile['Keyword or Product Targeting'].isin(Keyword_list)))&(dfbulkfile["Status"]=="enabled")&((dfbulkfile["Campaign Status"]=="paused")|(dfbulkfile["Ad Group Status"]=="paused")),"更新内容"]="转化率>0.2，开启：无关词关闭 "                                    
        dfbulkfile.loc[(dfbulkfile["Campaign"]==boi_campaign) & (dfbulkfile["Ad Group"]==boi_ADgroup)&(~(dfbulkfile['Keyword or Product Targeting'].isin(Keyword_list)))&(dfbulkfile["Status"]=="enabled")&((dfbulkfile["Campaign Status"]=="paused")|(dfbulkfile["Ad Group Status"]=="paused")), "Status"]="paused"

        print("测试取反",(~(dfbulkfile['Keyword or Product Targeting'].isin(Keyword_list))))
...
####记录
       # dfbulkmodifyrecord.loc[(dfbulkfile["Campaign"]==boi_campaign) & (dfbulkmodifyrecord["Ad Group"]==boi_ADgroup)&(~(dfbulkmodifyrecord['Keyword or Product Targeting'].isin(Keyword_list)))&(dfbulkmodifyrecord['Match Type']==boi_Matchtype),Columnnew2]="转化率>0.2，开启：无关词关闭 "
                                    
        #dfbulkmodifyrecord.loc[(dfbulkfile["Campaign"]==boi_campaign) & (dfbulkmodifyrecord["Ad Group"]==boi_ADgroup)&(~(dfbulkmodifyrecord['Keyword or Product Targeting'].isin(Keyword_list)))&(dfbulkmodifyrecord['Match Type']==boi_Matchtype),Columnnew1]="bocondition05df"
