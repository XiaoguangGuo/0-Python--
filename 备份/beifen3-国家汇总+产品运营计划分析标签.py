...
CampaignWeek1CampaignTotalCount["广告开启数量"]=CampaignWeek1CampaignTotalCount.loc[CampaignWeek1CampaignTotalCount["Campaign Status"]=="enabled","Campaign"]

print(CampaignWeek1CampaignTotalCount)
...
for plan_country in plan_country_List:
    ...
    CampaignWeek1WithEnabled_list=CampaignWeek1CampaignTotalCount.loc[CampaignWeek1CampaignTotalCount["Country"]==plan_country,"SKU"].to_list()
    ...
    for plan_Country_SKU in plan_country_SKU_list:
        ...
        if ',' in  plan_Country_SKU:
            ...
            print(plan_Country_SKU)
            SKUchaifenAlllist+=[plan_Country_SKU]
             

#################################################以下有问题#################################################################
        
        for skucahifen88 in SKUchaifenAlllist:
            if skucahifen88 in CampaignWeek1WithEnabled_list:
                plan_skucahifen88_list=CampaignWeek1CampaignTotalCount[(CampaignWeek1CampaignTotalCount["Country"]==plan_country)&(CampaignWeek1CampaignTotalCount["SKU"]==skucahifen88),"广告开启数量"].to_list()

                
                if len(plan_skucahifen88_list)>1:
                    plan_enabled_sum=0
                    for enabled_oi in range(0,len(plan_skucahifen88_list)):
                        plan_enabled_sum+=plan_skucahifen88_list[enabled_oi]
                else:
                    plan_enabled_sum=plan_skucahifen88_list[0]


                    
        plan.loc[(plan["SKU"]==plan_Country_SKU)&(plan["Country"]==plan_country), "广告开启数量"]=plan_enabled_sum
