...
countrylist1=["NEW-JP","NEW-IT","GV-MX","NEW-MX","NEW-FR"]
...
####从第一行开始遍历这个符合条件1的筛选表
    ...
    for boi in range(0,len(bocondition_new01df)):
        ...
        boi_keyword=bocondition_new01df.iloc[[boi],[2]].values[0][0]
        print(boi_keyword,boi)
         
        boi_campaign=bocondition_new01df.iloc[[boi],[1]].values[0][0]                                                                                            
...
#########a将修改过的文件存入bulkoperationfilesnewpath
    ...
    dfbulkfile_notnull.to_excel(bulkoperationfilesnewpath+"Re_"+Today+"_"+str(bulkoperationfile),index=False)#生成简化版
