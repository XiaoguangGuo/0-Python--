...
for file in os.listdir(src_dir_path_inventory):
    ...
    data_csv = pd.read_csv(src_dir_path_inventory+'\\'+ str(file),encoding='Latin1')
    # 读文件
#
    #US
    if key[0] in file:
    ...
    # 执行语句
        print("有US库存")
         ...
         # 旧语句data_csv = pd.read_csv(r'D:\\运营\\计划数据\\老站\\当日库存\\'+str(file),encoding='gbk', error_bad_lines=False)     # 读取以分
        print(data_csv)
...
for salesfile in os.listdir(salesfilepath):
    print(salesfile)
#遍历数据文件
    sourcedata=pd.read_excel(salesfilepath+str(salesfile)).assign(Country=os.path.basename(salesfile).split('_')[0], 日期=os.path.basename(salesfile).split('_')[1])
