...
##差广告：点击次数>16且订单数<点击次数/16 或者点击次数》12 且订单数<点击次数/12
    if bulkfileCountry in countrylist1:
        ...
        bocondition_new01df=AllbulkCampaignKeyword[(AllbulkCampaignKeyword["Country"]==str(bulkfileCountry)) & (AllbulkCampaignKeyword["Clicks"]>clickenough2)&(AllbulkCampaignKeyword["Orders"]<(AllbulkCampaignKeyword["Clicks"]/clickenough2).apply(np.floor))].reset_index(drop=True)
    #符合第一个条件是有点击且订单数>点击数/16.#取整替代方法尝试(AllbulkCampaignKeyword["Clicks"]/clickenough2).round(0) #.astype("int")
    else:
        bocondition_new01df=AllbulkCampaignKeyword[(AllbulkCampaignKeyword["Country"]==str(bulkfileCountry)) & (AllbulkCampaignKeyword["Clicks"]>clickenough1)&(AllbulkCampaignKeyword["Orders"]<(AllbulkCampaignKeyword["Clicks"]/clickenough1).apply(np.floor))].reset_index(drop=True)   
