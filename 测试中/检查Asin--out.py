...
File_FanchaAsinTerms=r'D:\运营\1数据源\反查关键词\\GV-US_B074C6F1T2_反查关键词-10.xlsx'
File_selfAsinTerms=r'D:\\运营\\1数据源\\反查关键词\\GV-US_反查关键词-B074C6F1T2-进阶版.xlsx'
selfAsinTerms=pd.read_excel(File_selfAsinTerms)
selfAsinTerms_formerge=selfAsinTerms[["关键词","搜索热度"]]
FanchaAsinTerms=pd.read_excel(r'D:\运营\1数据源\反查关键词\\GV-CA_反查关键词-10.xlsx')
...
output=pd.merge(FanchaAsinTerms,seartchtermSummary_df_grouped_country,left_on='关键词', right_on="Customer Search Term",how="outer") 
output=pd.merge(output,Top1M_df,left_on='关键词', right_on="Search Term",how="left")


output.to_excel(r'D:\运营\关键词考察\\' +"New"+sku+"GV-US_反查关键词-10-out.xlsx")
