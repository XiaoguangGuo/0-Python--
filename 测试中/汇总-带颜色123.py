...
def calculate_sales_summary(file_path):
    ...
    exchange_rates = pd.Series(fetch_exchange_rates(app_id, currencies))
    countrylist=["英国","日本","加拿大","意大利","德国" ,"西班牙","法国","美国"]

    df['销售额'] = df.apply(lambda x: x['销售额'] / exchange_rates[exchangerate_dic[country_dic[x['站点']]]] if x['站点'] in country_dic else x['销售额'], axis=1)
    ...
    with pd.ExcelWriter(r'D:\运营\\3数据分析结果\\SailingstarTotalWeek23.xlsx') as writer:
