...
AlldataWeekgroup=AdData.groupby(["周数","SKU3"]).agg({"Campaign":"nunique","Spend":"sum","Clicks":"sum","Impressions":"sum","Max Bid":"mean","Orders":"sum","Keyword or Product Targeting":"nunique"},index=False)

AlldataWeekgroup.reset_index(inplace=True)
...
has_missing_values = AlldataWeekgroup.isna().any().any()

print("分组统计是否有缺失值？", has_missing_values)


AlldataWeekgroup.rename(columns={"Campaign":"Campaign_unique","Spend":"Spend_sum","Clicks":"Clicks_sum","Impressions":"Impressions_sum","Max Bid":"Maxbid_average","Orders":"Orders_sum","Keyword or Product Targeting":"Keywords_unique"},inplace=True)

AlldataWeekgroup.loc[AlldataWeekgroup["Clicks_sum"]==0, "zhuanhualv"]=0
AlldataWeekgroup.loc[AlldataWeekgroup["Clicks_sum"]>0, "zhuanhualv"]=AlldataWeekgroup["Orders_sum"]/AlldataWeekgroup["Clicks_sum"]

Alldata=pd.merge(AlldataWeekgroup,Sales_df_taget, left_on=["SKU3","周数"],right_on=["SKU","周数"],how="left")

Alldata=Alldata.drop(["SKU","SKU3","周数"],axis=1)
...
data_encoded=Alldata.drop("Impressions_sum",axis=1)
...
# 创建并训练模型
dt = DecisionTreeRegressor(max_depth=3,min_samples_split=7,min_samples_leaf=4,random_state=42)
