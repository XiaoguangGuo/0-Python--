...
def find_blank_area(file_path, extra_rows=20):
    ...
    for row in range(1, max_row + extra_rows + 1):
        ...
        if empty_cells >= 8:
            # 判断是否在合并单元格内
            in_merged_cell = any([cell.min_row <= row <= cell.max_row and cell.min_col <= col <= cell.max_col and not isinstance(ws.cell(row=row, column=col), openpyxl.cell.read_only.ReadOnlyCell) for cell in merged_cells for col in range(1, max_col + 1)])
        ...
        if consecutive_blank_rows == 8:
            blank_area_start = row - 7
            break
