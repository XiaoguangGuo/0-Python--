...
def find_blank_area(file_path, extra_rows=10):
    ...
    for row in range(1, max_row + extra_rows + 1):
        ...
        for col in range(1, max_col + 1):
            ...
            if cell.value is None and not any(cell.coordinate in merged_range for merged_range in ws.merged_cells):
                empty_cells += 1
    ...
    if blank_area_start > 0 and blank_area_start - 1 >= 1:
        col_names = [ws.cell(row=blank_area_start - 1, column=col).value for col in range(1, max_col + 1) if not any(ws.cell(row=blank_area_start - 1, column=col).coordinate in merged_range for merged_range in ws.merged_cells)]
        print(col_names)
        return col_names
    else:
        return None
