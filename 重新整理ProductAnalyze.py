...
#将Product_Analyzefile_df中的日期列转换为周数并添加周数列
Product_Analyzefile_df=update_week_numbers(Product_Analyzefile_df)
#如果日期小于2023/5/8  0:00:00，周数加1 因为之前都是按周日为一周的最后一天，现在改为按周六为一周的最后一天
Product_Analyzefile_df.loc[pd.to_datetime(Product_Analyzefile_df["日期"]) < datetime(2023,5,8), "周数"] += 1
...
all_sales_df=all_sales_df[all_sales_df["周数"]<53]
all_advertise_df=pd.read_excel(r'D:\运营\2生成过程表\周bulk数据Summary.xlsx',sheet_name="SKU-WEEK")
all_advertise_df=all_advertise_df[all_advertise_df["周数"]<53]
#筛选Country列名只包含GV-US，GV-CA和GV-MX的行。
all_advertise_df = all_advertise_df[all_advertise_df['Country'].isin(["GV-US", "GV-CA", "GV-MX"])]
#去掉'Impressions','Clicks'和'Orders'都为0的行
all_advertise_df = all_advertise_df[~((all_advertise_df['Impressions'] == 0) & (all_advertise_df['Clicks'] == 0) & (all_advertise_df['Orders'] == 0))]
...
missing_asin_rows = all_advertise_df[all_advertise_df['ASIN'].isna()]

if not missing_asin_rows.empty:
    print("Rows with missing ASIN values:")
    print(missing_asin_rows)
    missing_asin_rows.to_excel(r'D:\运营\missing_asin_rows.xlsx')
    print("有ASIN缺省，表示有些做了广告但没有对应asin")


else:
    print("No rows with missing ASIN values.")
...
all_advertise_df=all_advertise_df[['Country', 'SKU', '周数', 'Impressions', 'Clicks', 'Spend', 'Orders',
       'Total Units', 'Sales','ASIN','MSKU']]

all_sales_df = pd.merge(all_sales_df, all_advertise_df, how='outer', left_on=['Country','SKU','周数'], right_on=['Country','SKU','周数'])


all_sales_df.loc[all_sales_df['SKU'].isna(), 'SKU'] = all_sales_df.loc[all_sales_df['SKU'].isna(), 'MSKU']

all_sales_df.loc[all_sales_df['(Child) ASIN'].isna(), '(Child) ASIN'] = all_sales_df.loc[all_sales_df['(Child) ASIN'].isna(), 'ASIN']
                                                                                                      
all_sales_df = all_sales_df.drop(columns=['MSKU','ASIN'])
...
Product_Analyzefile_df["广告浪费金额"]=Product_Analyzefile_df["广告花费"]-Product_Analyzefile_df["销量"]*1
Product_Analyzefile_df.to_excel(r'D:\运营\2生成过程表\TESTAll_Product_Analyzefilebeforepivot.xlsx')
Product_Analyzefile_df=Product_Analyzefile_df[['ASIN', '店铺', '站点','周数','Sessions', '销量', '销售额', '广告点击量','广告花费', '广告订单量', '广告销售额','毛利润','广告浪费金额']]
Product_Analyzefile_df["Sessions"].fillna(0,inplace=True)
# Create the pivot table
Product_Analyzefile_df_pivot = Product_Analyzefile_df.pivot_table(
    index=['ASIN', '店铺', '站点'],
    columns='周数',
    values=['Sessions', '销量', '销售额', '广告点击量','广告花费', '广告订单量', '广告销售额','毛利润','广告浪费金额'],
    aggfunc='sum'
)
...
for i in range(1, 16):
    Product_Analyzefile_df_pivot["第"+str(i)+"周入库"] = Product_Analyzefile_df_pivot["第"+str(i)+"周入库"].fillna(0)
    
for	i in range(1, 53):
    Product_Analyzefile_df_pivot["销量"+str(i)] = Product_Analyzefile_df_pivot["销量"+str(i)].fillna(0)
    Product_Analyzefile_df_pivot["广告订单量"+str(i)] = Product_Analyzefile_df_pivot["广告订单量"+str(i)].fillna(0)
    Product_Analyzefile_df_pivot["广告销售额"+str(i)] = Product_Analyzefile_df_pivot["广告销售额"+str(i)].fillna(0)
    Product_Analyzefile_df_pivot["广告点击量"+str(i)] = Product_Analyzefile_df_pivot["广告点击量"+str(i)].fillna(0)
...
Product_Analyzefile_df_pivot["4周广告浪费金额"]=Product_Analyzefile_df_pivot["广告浪费金额1"]+Product_Analyzefile_df_pivot["广告浪费金额2"]+Product_Analyzefile_df_pivot["广告浪费金额3"]+Product_Analyzefile_df_pivot["广告浪费金额4"]
...
Product_Analyzefile_df_pivot["Fufillable"].fillna(0, inplace=True)
Product_Analyzefile_df_pivot["Receiving"].fillna(0, inplace=True)
Product_Analyzefile_df_pivot["Reserved"].fillna(0, inplace=True)
...
Product_Analyzefile_df_pivot = update_weekly_demands(Product_Analyzefile_df_pivot, WeekSales)
Product_Analyzefile_df_pivot_group=Product_Analyzefile_df_pivot.groupby(["店铺","站点"]).agg(sum)
Product_Analyzefile_df_pivot_group.to_excel("D:/运营/2生成过程表/country_group.xlsx")
###########################################################3333
...
Product_Analyzefile_df_pivot.loc[(Product_Analyzefile_df_pivot["4周广告浪费金额"] >=20) & (Product_Analyzefile_df_pivot["广告浪费金额1"]>=8), "皮质层标签"] = Product_Analyzefile_df_pivot["皮质层标签"].astype(str)+"广告浪费严重-怪兽，"
